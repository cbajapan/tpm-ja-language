...
$lang = array (
	...
	'v_set_imp_num_prj_created'		=> '[PRJS]の作成数',
 	...
 	'v_set_timeout_gc_maxlifetime'	=> 'データがゴミとみなされ、クリーンアップされるまでの秒数を指定します。この設定はしばしば非アクティブなユーザセッションのタイムアウトとして使用されますが、PHPのガベージコレクタを使用するため、正確なタイムアウトとしては使用できません。これがTeam Password Managerが独自のセッションタイムアウトを実装する理由です。この設定は、少なくともタイムアウトフィールドで指定したものと同じ大きさにする必要があります。',
